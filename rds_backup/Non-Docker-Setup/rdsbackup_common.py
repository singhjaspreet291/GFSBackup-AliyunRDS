...
key=key[2:]
key=key[:-1]
password_enc = str(parser.get('enc_config', 'paraphrase'))
...
print(db_name)
AccessKeyId = str(parser.get('oss_config', 'accessid'))
AccessKeyId= str(base64.b64decode(AccessKeyId))
AccessKeyId=AccessKeyId[2:]
AccessKeyId=AccessKeyId[:-1]
AccessKeySecret = str(parser.get('oss_config', 'accesskey'))
